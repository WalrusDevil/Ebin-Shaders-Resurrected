...
vec3 Compute2DCloudPlane(vec3 wDir, vec3 wPos, vec3 transmit, float phase) {
	...
	float coverage = CLOUD_COVERAGE_2D * 1.16;
	coverage = mix(coverage, 1.0, rainStrength);
	...
	//cloudAlpha = mix(cloudAlpha, 0.1, rainStrength);
	...
	sunlight *= phase ;
	sunlight *= 1000.0;
	sunlight = mix(sunlight, sunlight / 4, rainStrength);
